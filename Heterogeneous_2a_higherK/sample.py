...
def fehm(p):
    ...
    with open('perm_base.dat','w') as fh:
        ...
        fh.write("    %d %d %d %e %e %e\n"%(27581, 50990, 2601, p['perm7'], p['perm7'], p['perm7']))
        fh.write("    %d %d %d %e %e %e\n"%(28091, 51500, 2601, p['perm8'], p['perm8'], p['perm8']))
        fh.write("    %d %d %d %e %e %e\n"%(26031, 49440, 2601, p['perm9'], p['perm9'], p['perm9']))
...
p.add_par('perm7',min=1e-19,max=1e-14)
p.add_par('perm8',min=1e-19,max=1e-14)
p.add_par('perm9',min=1e-19,max=1e-14)
...
s.run(cpus=8, workdir_base='workdir', outfile='results.dat', logfile='log.dat',verbose=False,reuse_dirs=True,restart_logfile='log_restart.dat')
