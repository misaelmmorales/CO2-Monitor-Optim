...
Main_Directory = '/scratch/sft/bailian_chen/Heterogeneous_2a_higherK'
nTrain = 500                                # The number of training simulations
Obj_filename = 'run_co2mt.his'               # Objective file name
NLeakPotential = 6                           # Number of potential leak point 
nColumn_obj = 50                             # The column(s) in which the object of interest is located

MeasureType = 1                                # Measurement type: 1 for pressure; 2 for CO2 saturation; 3 for tempeture
nColumn_data = [3,12]                           # The column in which the data measurement is located 
...
nMCSamples = 50000                           # Number of monte carlo samples
...
if MeasureType == 1:    # for pressure
    ...
    eps = 0.005                                  # History match error tolerance
    ...
    for itrain in range(0, nTrain):
        ...
        for lines in open(train_data_filename):
            ...
            if count>5:
                ...
                data_train_read_raw[itrain][count-6+i*nTimeSeries] = xi
time_point[1:nTimeSeries+1] = time_set[:nTimeSeries]
data_train_read = data_train_read_raw.T
data_train=data_train_read
data_train_scaled = np.zeros_like(data_train)
for i in range(0,nData):
    data_train_scaled[i] = (data_train[i]-np.min(data_train[i]))/(np.max(data_train[i])-np.min(data_train[i]))
...
# Scale x_train to [0,1]
x_train_scaled = np.zeros((nTrain,nParam))
for i in range(0,nTrain):
    for j in range(0,nParam):
        x_train_scaled[i][j] = (x_train[i][j] - x_min[j])/(x_max[j] - x_min[j])
...
if ROMs_validation==1:
    ...
    predict_obj_scaled = np.zeros(nTrain)
    predict_data_scaled = np.zeros((nData,nTrain))
    predict_data = np.zeros_like(predict_data_scaled)
    for i in range(0,10):
        x_train_scaled_v=x_train_scaled.tolist()
        y_train_v=y_train_scaled.tolist()
        data_train_raw_v=data_train_scaled.T.tolist()
        ...
        # predict   
        predict_obj_scaled[Interval*i:Interval*(i+1)] = ROM_obj.predict(x_train_scaled[Interval*i:Interval*(i+1)])
        for iData in range(0,nData):
            predict_data_scaled[iData][Interval*i:Interval*(i+1)] = ROM_data[iData].predict(x_train_scaled[Interval*i:Interval*(i+1)])
    for i in range(0,nData):
        for j in range(0,nTrain):
            if predict_data_scaled[i][j]<0:
                predict_data_scaled[i][j]=0
            if predict_data_scaled[i][j]>1:
                predict_data_scaled[i][j]=1
            predict_data[i][j]=predict_data_scaled[i][j]*(np.max(data_train[i])-np.min(data_train[i]))+ np.min(data_train[i])
        
    for i in range(0,nTrain):
        if predict_obj_scaled[i]<0:
            predict_obj_scaled[i]=0
    predict_obj = predict_obj_scaled*(np.max(y_train)-np.min(y_train))+np.min(y_train)
    ...
    for i in range (0,nData):        
        ...
        plt.xlabel('ROM Prediction (MPa)',fontsize=16,fontweight="bold")
        plt.ylabel('True Value from Simulation (MPa)',fontsize=16,fontweight="bold")
    ...
    plt.xlabel('Monitoring Data Point',fontsize=16,fontweight="bold")
    ...
    real_max1=max(y_train/1e6)
    pred_max1=max(predict_obj/1e6)
    maxV1=max(real_max1,pred_max1)+5
    plt.plot([0,maxV1],[0,maxV1],ls="-",c="0.3")
    plt.xlabel('ROMs Prediction (Kt)',fontsize=16,fontweight="bold")
    plt.ylabel('True Value from Simulation (Kt)',fontsize=16,fontweight="bold")
    plt.rc('xtick',labelsize=14)
    plt.rc('ytick',labelsize=14)
    plt.xlim([0,maxV1])
    plt.ylim([0,maxV1])
    plt.savefig("ROMs-obj",bbox_inches='tight')
    plt.show()
...
print 'Evaluate Monte Carlo samples: ing... ing ...'
...
for i in range(0,nDataRealization):
    for j in range(0,nParam):
        ...
        LHSamples_scaled[i][j] = (LHSamples[i][j] - x_min[j])/(x_max[j] - x_min[j])
...
post_p90mp10_mean, post_p90mp10_time, post_p90mp10_iData, post_mean, post_mean_iData, nSamples,mc_obj_post = Uncertainty_calc(mc_data,synthetic_data,mc_obj,err_option,eps,time_sensitivity,len(nColumn_data))
...
if post_processing==1:
    ...
    plt.plot(np.array(time_point)/360,U,marker='+',color='red')
    ...
    for iLoc in range(0, len(nColumn_data)):
        ...
        figname = 'data_realizations_Loc_'+str(iLoc+1)
        plt.savefig(figname,bbox_inches='tight')
        plt.show()
